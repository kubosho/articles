...
このブログには元々ダークモードがありませんでした。[はてなブログから独自のシステムに移行した2019〜2020年](https://blog.kubosho.com/entries/migrating-from-contentful-to-markdown-file)は、Android 10やiOS 13でダークモードへの切り替えがリリースされて、ようやくダークモードが一般層にも広がる素地ができた年でした。
...
ポップオーバーの位置決めはJavaScriptで位置計算をして実装することが多かったですが、Anchor Positioningを使うことによってCSSだけでトリガーボタンを基準にポップオーバーを配置できます。

UI実装で標準搭載されているAPIを使うことで、機能面の実装に集中できるのが良いですね。

---

モードの設定はCookieに保存しています。ページ読み込み時にCookieからテーマを読み取り、html要素の `data-theme` 属性に反映します。この処理は`<head>`内のインラインスクリプトで行い、FOUC（Flash of Unstyled Content）を防いでいます。
...
色の定義にはOKLCHを採用しました。従来のHEX値やHSLではなく、OKLCHを選んだ理由は知覚的な均一性です。

OKLCHの色空間を使うことで、色を比較したときに一方の色が沈んで見えたり、色が違って見えるといった問題が起きにくくなります。また、[OKLCH Color Picker & Converter](https://oklch.com/)といった直感的な色調整ツールがあるのも良いです。

ただ、色の選定には想定以上に時間がかかりました。ライトモードの色をそのまま反転させても違和感のある配色になります。結局、ライトモードとダークモードの両方とも一から色を選び直しました。

なお、一部の色はセマンティックトークンとして定義しています。`--color-neutral-background`や`--color-neutral-text`のように、用途で名前をつけることで、ライトモードとダークモードで異なる値を割り当てやすくなります。
...
今回の調整で一番苦労したのはメインの色です。今までは `#003760`（OKLCHでは `oklch(0.329 0.0888 248.18)`）を使っていましたが、ダークモードの背景 `oklch(0.28 0 0)` と合わせたときにコントラストがLc 0となってしまうため色を変えなくてはいけませんでした。個人的には深みのある青色の象徴として `#003760` という値を暗唱できるくらいには気に入っていましたが、色を調整してライトモードでは `oklch(0.43 0.119 253)` を使い、ダークモードでは `oklch(0.51 0.141 253)` を使うようにしました。

はじめは違和感がありつつ、今は見慣れました。人間は慣れるものですね。
